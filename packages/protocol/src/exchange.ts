 ...
 * 
 * Either add messages in order one at a time using {@link Exchange.addMessage},
...
export class Exchange {
  ...
  addNextMessage(message: Message): void {
    ...
    if (this.exchangeId !== undefined && message.metadata.exchangeId !== this.exchangeId) {
      throw new Error(
        `Could not add message with id ${message.metadata.id} to exchange because it does not have ` +
  ...
  /**
   * @returns true if the Exchange has a Close message, false otherwise
   */
